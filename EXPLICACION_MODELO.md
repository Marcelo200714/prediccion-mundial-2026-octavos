...
documento corresponden a una ejecución real del pipeline (`python src/main.py`)
...
| Métrica | Valor |
|---|---|
| Log-loss con temperatura neutra (T=1) | **1.0064** |
| Log-loss con la mejor temperatura encontrada (T=0.90) | **1.0058** |
...
1. **Validación temporal, no aleatoria.** Se ordenan los 7.617 partidos de
   entrenamiento por fecha, y se separa el **15% más reciente** (1.143
   partidos) como conjunto de validación. El modelo se entrena solo con el
   85% más antiguo (6.474 partidos) y se evalúa contra ese 15% que nunca vio.
...
3. **Una vez medida la calidad, se reentrena con el 100% de los datos** (los
   7.617 partidos completos) para el modelo que efectivamente se usa a la
   hora de predecir los partidos de Octavos de Final pendientes. La métrica
   de validación (1.0058) es la mejor estimación honesta de qué tan bien
   generalizará ese modelo final.
...
| Partidos usados para entrenar (ambos equipos del Mundial 2026, resultado conocido) | 7.617 |
| Partidos de Octavos de Final ya jugados (resultado real, no se predicen) | 2 |
| Partidos de Octavos de Final pendientes de predecir | 6 |
| Variables finales usadas por el modelo | 19 |
| Simulaciones de Monte Carlo por partido | 10.000 |
| Temperatura óptima encontrada | 0.90 |
...
1.0058**, una mejora modesta pero real sobre el "no saber nada" (1.0986) —
