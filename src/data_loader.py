"""
Carga y unificacion de fuentes crudas (sec. 1 de 'Cerebro_Del_Modelo.md').
...
los 6 partidos de Octavos de Final AUN NO JUGADOS (con marcador NaN). Es decir,
'results.csv' es exactamente el log historico + actual, sin fuga de futuro
(los resultados que no se conocen todavia aparecen en NaN). Esto nos permite
usarlo como la unica fuente de "goles" para el entrenamiento (1872-2026),
mientras que 'matches.csv' + 'match_team_stats.csv' aportan estadisticas mas
ricas (xG, posesion, tiros, corners, paradas) SOLO disponibles para los
partidos de este Mundial 2026.
"""
import pandas as pd

from config import (
    MATCHES_CSV, TEAMS_CSV, MATCH_TEAM_STATS_CSV, RESULTS_CSV,
    SHOOTOUTS_CSV, SQUADS_CSV, MAPEO_NOMBRES,
)
...
def cargar_valor_mercado() -> pd.DataFrame:
    """Suma el valor de mercado de la plantilla por equipo (proxy de
    'Valor_Mercado_Millones_Eur', sec. 1.3)."""
...
def resolver_ganador_penaltis(historico: pd.DataFrame, shootouts: pd.DataFrame) -> pd.DataFrame:
    ...
    replicando 'Resultado_Penaltis' de la sec. 1.3."""
    df = historico.merge(
...
def construir_matches_2026_enriquecido(teams: pd.DataFrame) -> pd.DataFrame:
    ...
    (sec. 3 del enunciado: cruzar CSVs por sus llaves)."""
